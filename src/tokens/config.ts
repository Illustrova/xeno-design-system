...
StyleDictionary.registerFormat({
  ...
  formatter: ({ dictionary, file }) => {
    const res = `export const tokens = {${colorObjectToStringWithJsDoc(
      dictionary.tokens
    )}}`;
    return res;
  },
...
function colorObjectToStringWithJsDoc(obj: TransformedTokens) {
    ...
    if (!obj[name].hasOwnProperty("value")) {
      ...
      if (typeof token.value === "object") {
        toRet += `\n/**\n\`\`\`${JSON.stringify(
          ...
          null,
          2
        )}\`\`\` \n*/\n\"${name}\": ${JSON.stringify(token.value)},`;
      } else if (
        _.isNumber(token.value) ||
        (_.isString(token.value) && token.value.length > 0)
      ) {
        toRet += `\n/** ${token.value} */\n\"${name}\": \"${token.value}\",`;
      }
    }
